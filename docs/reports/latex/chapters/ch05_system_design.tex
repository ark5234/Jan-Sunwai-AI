...
\begin{description}
\item[\texttt{users}:] Uses \texttt{unique(username)} and \texttt{unique(email)} for fast auth lookups and enforcing uniqueness.
\item[\texttt{complaints}:] Uses compound indices like \texttt{(user\_id, created\_at)} for citizen dashboards, \texttt{(status, created\_at)} for admin queues, and \texttt{(department, created\_at)} for department views. \texttt{authority\_id} is indexed for escalation queries.
\item[\texttt{notifications}:] Uses \texttt{(user\_id, created\_at)} for feed retrieval and \texttt{(user\_id, is\_read)} for unread badge counters.
\item[\texttt{password\_resets}:] Uses \texttt{unique(token\_hash)} for token lookups, \texttt{(user\_id, used)} for invalidation, and a \texttt{TTL(expires\_at)} index for automatic cleanup.
\end{description}
...
\begin{description}
\item[Public Home:] Provides awareness, transparency access, and role entry points.
\item[Authentication:] Manages login, registration, and password recovery flows.
\item[Analyze \& Result:] Handles image uploads, AI analysis display, interactive maps, and draft review.
\item[Citizen Dashboard:] Displays the personal complaint list, lifecycle timeline, feedback, and progress visibility.
\item[Worker Dashboard:] Shows the assigned task queue and allows completion updates.
\item[Department Head Dashboard:] Provides department-level filtering, status updates, and note-taking.
\item[Admin Dashboard:] Provides global queue access, worker approvals, triage, bulk actions, analytics, and exports.
\item[Notifications \& Profile:] Manages event feeds, read-states, and personal information updates.
\end{description}
